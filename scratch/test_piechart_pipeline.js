...
const finalAnswer = `មុំសម្រាប់ចំណិតនីមួយៗនៃតារាងចំណិតជុំគឺ:
*   គណិតវិទ្យា: $144^\\circ$
*   វិទ្យាសាស្ត្រ: $108^\\circ$
*   ភាសាអង់គ្លេស: $72^\\circ$
*   ប្រវត្តិវិទ្យា: $36^\\circ$`;

const solutionText = `ដើម្បីគូរតារាងចំណិតជុំ (pie chart) ...`;
...
function inferPieChartBlocks(problem, finalAnswer, solutionText) {
  ...
  const isPieChart = /(pie.?chart|piechart|តារាងចំណិតជុំ|ចំណិតជុំ|រង្វង់ចំណិត)/i.test(source);
  ...
  for (const line of lines) {
    ...
    if (match1) {
      ...
      if (label && Number.isFinite(pctVal) && pctVal > 0 && pctVal <= 100) {
        if (!/^(min|max|q1|q2|q3|minimum|maximum|total|ភាគរយសរុប|មុំសរុប|មុំ|មេដ្យាន)$/i.test(label)) {
          const dupIdx = sectors.findIndex((s) => s.label.toLowerCase().includes(label.toLowerCase()) || label.toLowerCase().includes(s.label.toLowerCase()));
          if (dupIdx !== -1) {
    ...
    const match2 = line.match(/(?:[*+-]|\b)\s*(\d+(?:\.\d+)?)\s*\\?%\s*(?:like|of|)\s*([^\n]+)/i);
    if (match2) {
      const pctVal = parseFloat(match2[1]);
      const label = match2[2].replace(/[$*{}]/g, "").trim();
      if (label && Number.isFinite(pctVal) && pctVal > 0 && pctVal <= 100) {
        if (!/^(min|max|q1|q2|q3|minimum|maximum|total|ភាគរយសរុប|មុំសរុប|មុំ|មេដ្យាន)$/i.test(label)) {
          const dupIdx = sectors.findIndex((s) => s.label.toLowerCase().includes(label.toLowerCase()) || label.toLowerCase().includes(s.label.toLowerCase()));
          if (dupIdx !== -1) {
...
const result = inferPieChartBlocks(problem, finalAnswer, solutionText);
